				     ...
				     struct fb_info *info)
{
	...
	if (boss_fb->active) {
		...
		int offset;
                ...
                dst1 = dst2 = src;
                display_osd_switch(voutid, dst1, dst2);
	}
...
#if defined(ARGB8888)
	// Force ARGB8888 mode //
	boss_fb->bossed.colorformat = 31;//// OSD_FORMAT_ARGB_8888
	boss_fb->bossed.bitsperpixel = 32;
#endif
	
#if defined(RGB565)
	// Force RGB565 mode //
	boss_fb->bossed.colorformat = VDSPDRV_COLOR_RGB565;//OSD_FORMAT_VYU_RGB_565
...
/*#if defined(ARGB8888)
	colorformat = AMBFB_COLOR_ARGB8888;
	bytesperpixel = 4;
#endif
	...
	bytesperpixel = 2;
#endif*/
	if(pfb->color_format == AMBFB_COLOR_ARGB8888 ){
		colorformat = AMBFB_COLOR_ARGB8888;
		bytesperpixel = 4;
	}else if (pfb->color_format == AMBFB_COLOR_RGB565){
		colorformat = AMBFB_COLOR_RGB565;
		bytesperpixel = 2;
	}else{
		DEBUG_MSG_FB ("[ipc] ERROR: color_format(%d) is NOT supported \n", pfb->color_format);
		return -EIO;
	}
