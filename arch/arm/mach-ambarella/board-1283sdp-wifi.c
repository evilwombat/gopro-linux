...
#include "board-1283sdp-wifi.h"
//#include "mux.h" only for omap chip
#define tristan_amba 1
...
int omap_wifi_status_register(void (*callback)(int card_present,void *dev_id), void *dev_id)
{
	...
	printk("omap_wifi_status_register SDIO notify functin\r\n");
	if (wifi_status_cb)
		return -EAGAIN;
...
static struct resource sdp4430_wifi_resources[] = {
	[0] = {
		...
		.start		= GPIO_INT_VEC(AMTI_WIFI_IRQ_GPIO),
		.end		= GPIO_INT_VEC(AMTI_WIFI_IRQ_GPIO),
